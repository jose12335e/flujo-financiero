...
export function StatCard({ caption, icon: Icon, tone = 'neutral', title, value }: StatCardProps) {
  return (
    ...
    >
      <div className="relative min-h-[10.5rem] pr-16">
        <div
          className={cn('absolute right-0 top-0 flex h-10 w-10 items-center justify-center rounded-xl', {
        ...
        <div className="max-w-full">
          ...
          <p className="mt-2.5 max-w-full text-[clamp(1.55rem,1.65vw,2.25rem)] leading-none font-bold tracking-tight text-text-primary">
            {value}
          </p>
          <p className="mt-3 max-w-[22ch] text-[0.95rem] leading-6 text-text-muted">{caption}</p>
        </div>
      </div>
    </Card>
  )
